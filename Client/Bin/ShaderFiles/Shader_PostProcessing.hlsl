...
struct PS_OUT_LIGHT 
{
	...
	vector		vShadowDepth : SV_TARGET2;
};
...
PS_OUT_LIGHT PS_MAIN_LIGHT_DIRECTION(PS_IN In)
{
	...
	////Out.vShadowDepth = vector(vLightWVP.z / vLightWVP.w, vLightWVP.w / g_fLightFar, 0.0f, 1.0f);
	////Out.vShadowDepth = vector(0.f, vLightWVP.w / g_fLightFar, 0.0f, 1.0f);
	//Out.vShadowDepth = vector(vLightWVP.z / vLightWVP.w, 0.f, 0.0f, 0.0f);
...
PS_OUT PS_MAIN_BLEND(PS_IN In)
{
	...
	//////////////////////////////////
	// Shadow
	//vector		vShadowDepth = g_texShadowDepth.Sample(DefaultSampler, In.vTexUV);
	...
	//{
	//	float fShadowDepth = vShadowDepth.r;
...
PS_OUT PS_POSTPROCESSING_BEFOREUI(PS_IN In)
{
	...
	Out.vColor.rgb = vDiffuse.rgb * g_fSourceRate + vFilter0.rgb * g_fOtherRate;

...
BOOL CToolApp::OnIdle(LONG lCount)
{
	...
	{
		m_dTimeAcc += pGameInstance->Compute_Timer(L"TimerDefault");

		if (m_dTimeAcc >= 1.0 / 60.0)
		{
			m_dTimeAcc = 0.0;
			_double dTimeDelta = pGameInstance->Compute_Timer(L"Timer60");

			pToolView->Tick(dTimeDelta);
			pToolView->LateTick(dTimeDelta);
			pGameInstance->Tick(dTimeDelta);
			pToolView->Render();
		}
	}
